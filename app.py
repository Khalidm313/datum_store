        ...
        if user and check_password_hash(user.password, password):
            if not user.is_admin and user.shop:
                if not user.shop.is_active:
                    ...
                    return redirect(url_for('login'))
                if user.shop.subscription_end and user.shop.subscription_end < datetime.now():
...
def register():
    if request.method == 'POST':
        ...
        # --- التحقق من أن اسم المتجر غير مكرر ---
        if Shop.query.filter_by(name=shop_name).first():
            flash('عذراً، اسم المتجر هذا مستخدم مسبقاً. يرجى اختيار اسم آخر.', 'error')
        ...
        # عند إنشاء متجر جديد، نعطيه فترة تجريبية 14 يوم
...
def dashboard():
    ...
    todays_invoices = Invoice.query.filter(Invoice.shop_id==shop_id, db.func.date(Invoice.date) == today, Invoice.status != 'refunded').all()
    todays_sales = sum(inv.total_amount for inv in todays_invoices)
    
    month_invoices = Invoice.query.filter(Invoice.shop_id==shop_id, db.func.extract('month', Invoice.date) == today.month, Invoice.status != 'refunded').all()
    month_sales = sum(inv.total_amount for inv in month_invoices)
    
    month_expenses_q = Expense.query.filter(Expense.shop_id==shop_id, db.func.extract('month', Expense.date) == today.month).all()
    month_expenses = sum(exp.amount for exp in month_expenses_q)
    ...
    chart_data = [100, 150, 120, 200, 180, 220, 300] 
...
def products():
    if request.method == 'POST':
        ...
        new_prod = Product(name=name, barcode=barcode, category=category, stock=stock, 
                           buy_price=buy_price, sell_price=sell_price, tax=float(tax), shop_id=current_user.shop_id)
...
def expenses():
    if request.method == 'POST':
        ...
        new_exp = Expense(description=desc, amount=amount, shop_id=current_user.shop_id)
...
def admin_dashboard():
    ...
    for s in shops:
        ...
        if s.subscription_end:
            if s.subscription_end > datetime.now():
                sub_status = "active"
                days_left = (s.subscription_end - datetime.now()).days
            else:
                sub_status = "expired"
...
def renew_subscription():
    ...
    return redirect(url_for('view_subscription_receipt', sub_id=new_sub.id))

@app.route('/admin/receipt/<int:sub_id>')
@login_required
def view_subscription_receipt(sub_id):
    if not current_user.is_admin: return redirect(url_for('login'))
    ...
    receipt_data = {
        'shop_name': shop.name,
        'amount': sub.amount,
        'payment_date': sub.created_at.strftime('%Y-%m-%d'),
        'payment_time': sub.created_at.strftime('%I:%M %p'),
        'start_date': sub.start_date.strftime('%Y-%m-%d'),
        'end_date': sub.end_date.strftime('%Y-%m-%d'),
        'transaction_id': f"SUB-{sub.id}",
        'plan_name': sub.plan_name
    }
    return render_template('subscription_receipt.html', **receipt_data)
...
@app.route('/admin/shop/toggle_rent/<int:id>')
@login_required
def toggle_shop_rent(id):
    if not current_user.is_admin: return redirect(url_for('login'))
    shop = Shop.query.get_or_404(id)
    shop.rent_paid = not shop.rent_paid
    db.session.commit()
    return redirect(url_for('admin_dashboard'))

@app.route('/admin/shop/edit/<int:id>', methods=['GET', 'POST'])
@login_required
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not User.query.filter_by(username='admin').first():
            ...
            admin = User(username='admin', password=hashed, role='admin', is_admin=True)
